...
nabízí možnost taková záhlaví hromadnì editovat, pøípadnì úplnì nahradit jiným záhlavím. 
Samozøejmostí je i možnost vložení nového záhlaví do souborù, které zatím žádné neobsahují.
...
\paragraph*{Dva zpùsoby výbìru souborù.}
Soubory zobrazené ve stromu souborù lze vybrat dvìma zpùsoby. 
První možnost je kliknutí na jméno souboru, potom text se jménem souboru zmìní barvu a 
...
\uv{Isert header} nastavit jako nové záhlaví vybraných souborù, 
...
Pøi tvorbì nového záhlaví lze použít pouze speciální promìnnou \mbox{'\%filename\%'}, která oznaèuje jméno souboru.
...
Uživatel mùže zmìnit obsah aktuálního záhlaví, potom jej pomocí toho pøíkazu uložit a
program zmìní obsah záhlaví u všech souborù, které jej obsahovaly. 
(Oznaèení souborù pomocí checkboxù tedy v tomto pøípadì nemá vliv a soubory, které budou zpracovány, jsou urèeny podle shodnosti záhlaví.)
To, že tato zmìna probíhá na záhlavích, která byla extrahována z existujících souborù, znamená, 
...
\begin{table}[h]
  ...
  \\
  \end{tabular}
  \caption{Jak záhlaví zobrazí program SourceHeader a jak vypadá ve skuteènosti. \label{code1}}  
\end{table}

\subparagraph*{}
Na obsah nìjakého alternujícího bloku se uživatel v textu záhlaví mùže odvolat pomocí 
sekvence \mbox{'\%jmeno-bloku\%cislo\%'}, kde \uv{jmeno-bloku} je oznaèení alternující èásti, 
...
V tabulce èíslo \ref{code1} je uvedena ukázka jak program zobrazí takové záhlaví a jak mùže vypadat ve skuteènosti.
...
\begin{figure}[h]
  ...
  \caption{Screenshot dialogu s nastavením.}
  \label{screen2}
\end{figure}

\paragraph*{}
Program umožòuje základní nastavení v grafickém rozhraní. 
Dialog s nastavením (obrázek \ref{screen2}) se zobrazí po pøíkazu z menu \uv{File}$\rightarrow$\uv{Preferences}.
Uživatel má možnost zmìnit speciální znak, který je defaultnì nastaven na '\%',
a cestu k souboru s konfigurací alternujících blokù. 
Zmìny konfigurace se projeví až po restartu programu.

\subparagraph*{}
Konfiguraci jednotlivých alternujících blokù lze upravovat pouze ruènì. 
Po stisknutí tlaèítka \uv{Insert default config} vygeneruje program do 
zadaného souboru defaultní nastavení spoleènì s nápovìdou k jeho syntaxi.
Pokud soubor s konfigurací alternujících èástí není uveden, program vnitønì 
naète defaultní nastavení alternujících blokù.
